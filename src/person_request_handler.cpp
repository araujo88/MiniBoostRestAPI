...
void PersonRequestHandler::handleRequest(const http::request<http::string_body> &request, http::response<http::string_body> &response)
{
    ...
    else if (request.method() == http::verb::post && request.target() == "/person")
    {
        try
        {
